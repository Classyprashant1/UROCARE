...
export default async function HomePage() {
  ...
  return (
    <Reveal className="overflow-hidden">
      {/* ============ HERO ============ */}
      <section className="relative min-h-[620px] flex flex-col pt-20">
        <div className="absolute inset-0 bg-gradient-to-br from-blue-800 via-teal-700 to-blue-900 z-0 overflow-hidden">
          <div className="absolute inset-0 bg-[url('data:image/svg+xml,%3Csvg width=\'60\' height=\'60\' viewBox=\'0 0 60 60\' xmlns=\'http://www.w3.org/2000/svg\'%3E%3Cpath d=\'M30 0 L30 60 M0 30 L60 30\' stroke=\'rgba(255,255,255,0.04)\' stroke-width=\'1\'/%3E%3C/svg%3E')] opacity-50"></div>
          <div className="absolute top-[20%] left-[20%] w-[60%] h-[60%] bg-blue-400/20 rounded-full blur-[120px]"></div>
        </div>
        
        <div className="relative z-10 flex-1 flex items-center pt-16 pb-28 max-w-7xl mx-auto w-full px-4 md:px-8">
          <div className="reveal opacity-0 translate-y-10 transition-all duration-700 max-w-2xl p-10 md:p-12 rounded-3xl bg-white/10 backdrop-blur-md border border-white/20">
        ...
        <div className="relative z-20 max-w-7xl mx-auto w-full px-4 md:px-8 -mt-12">
          <div className="grid grid-cols-1 md:grid-cols-4 gap-4 p-6 md:p-8 bg-white/90 backdrop-blur-xl border border-slate-200 shadow-2xl rounded-2xl items-end">
            <div className="flex flex-col gap-1.5">
              <label className="text-xs font-bold uppercase tracking-wider text-slate-500">Department</label>
              <select className="h-11 px-3 border border-slate-200 rounded-lg text-sm bg-white text-slate-700 focus:border-blue-600 focus:outline-none transition-colors w-full">
            ...
            <div className="flex flex-col gap-1.5">
              ...
              <input type="date" className="h-11 px-3 border border-slate-200 rounded-lg text-sm bg-white text-slate-700 focus:border-blue-600 focus:outline-none transition-colors w-full" />
            </div>
            <Link href="/booking" className="h-11 flex items-center justify-center px-6 bg-blue-700 text-white rounded-lg text-sm font-bold shadow-md shadow-blue-700/20 hover:-translate-y-0.5 transition-all w-full">
              Search Availability
            </Link>
          </div>
        </div>
